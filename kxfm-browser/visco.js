...
function process_graph_element( elm , current_dist, current_rank , total_ranks , direction , tag )
{
  ...
  const exitedBefore = elm.classList.contains( "exit" );
  const isLogicalAndNode = [ '&amp;', '∧', '+' ].indexOf(  elm.querySelector( "g a text" )?.innerHTML  ) > -1
...
F : { f : function(document)  // backward compatibility
      ...
      filter : (document) => hasSelection( document )
      ,
      text : "same as 'f' (lower case)"
    } ,

// in older browser versions lower-case key 'f' also captures CTRL-F, so historically there was no more find-in-page possible if capturing it
...
function on_svg_load( dom, options = {} )
{
  ...
  const sd = new SubDocument( dom );

  if (  sd.document.classList?.contains( 'kts_initialized' )  ) return;
